...
func test_reset_world_spawns_full_window() -> void:
	_cm.reset_world()
	assert_eq(
		_cm.active_chunk_count(),
		Constants.CHUNKS_AHEAD,
...
func test_scrolling_recycles_chunks() -> void:
	...
	assert_almost_eq(
		_cm.active_chunk_count(), initial, 2, "Window size stays bounded while scrolling"
	)
...
func test_obstacle_scene_pool_is_prewarmed() -> void:
	assert_gt(
		(
			PoolManager.free_count(ChunkManager.OBSTACLE_PATH)
			+ PoolManager.active_count(ChunkManager.OBSTACLE_PATH)
		),
		0,
