...
func _load_into_ui() -> void:
	if master_slider:
		master_slider.value = SettingsManager.get_value("master_volume") * 100.0
	if music_slider:
		music_slider.value = SettingsManager.get_value("music_volume") * 100.0
	if sfx_slider:
		sfx_slider.value = SettingsManager.get_value("sfx_volume") * 100.0
	if vibration_check:
		vibration_check.button_pressed = SettingsManager.get_value("vibration")
	if shake_check:
		shake_check.button_pressed = SettingsManager.get_value("screen_shake")
	if fps_check:
		fps_check.button_pressed = SettingsManager.get_value("show_fps")
	if contrast_check:
		contrast_check.button_pressed = SettingsManager.get_value("high_contrast")
